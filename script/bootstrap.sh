...
source /dev/stdin <<<"$(curl --silent https://raw.githubusercontent.com/umemotsu/dotfiles-centos/main/script/utils.sh)"
...
if [ -d "${DOT_ROOT}" ]; then
    ...
    run_verbosely git clone https://github.com/umemotsu/dotfiles-centos.git "${DOT_ROOT}"
fi
